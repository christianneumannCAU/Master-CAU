...
for s = 1:numel(nms_ids)
    ...
    target{s,5}     = beta_id{s}(dsearchn(depth_id{s},0));
    nf_beta{s,2}    = depth_id{s}(dsearchn(depth_id{s},10));
    target{s,6}     = beta_id{s}(dsearchn(depth_id{s},10));
...
%% safe for R 
writetable(T,'regression_table_exploring.csv');
writetable(target_dist_idx,'ttest_table_exploring.csv');
